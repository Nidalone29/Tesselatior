...
StaticModel* StaticModelCreator::CreateMesh(
  ...
  for (int i = 0; i < p_scene->mNumMeshes; i++) {
    ...
    /* const */ MESH_TYPE current_mesh_type = MESH_TYPE::TRI;
...
SubDivMesh* SubDivMeshCreator::CreateMesh(
  ...
  const aiScene* p_scene = importer.ReadFile(
      model_path_.string().c_str(),
      flags | aiProcess_JoinIdenticalVertices | aiProcess_DropNormals);
  ...
  for (Uint j = 0; j < pai_mesh->mNumFaces; j++) {
    ...
    switch (current_mesh_type) {
      case MESH_TYPE::TRI:
        ...
        break;
      case MESH_TYPE::QUAD:
        // TODO refactor this caused by old bug
        current_indices = {0, 1, 2, 3};
        break;
      default:
        // TODO unsupported polygon type
